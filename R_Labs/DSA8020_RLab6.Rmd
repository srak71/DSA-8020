...
ggplot(teengamb, aes(x = income, y = gamble)) +
  ...
  labs(title = "Scatterplot (amt gambled by income)", x = "income", y = "amt gambled") +
  theme_minimal()
```
...
ggplot() +
  ...
  geom_ribbon(data = data_plt, aes(x = income, ymin = lower, ymax = upper), alpha = 0.2) +
...
smooth_spline_fit <- smooth.spline(teengamb$income, teengamb$gamble, cv = TRUE)
...
gam_data <- data.frame(
  income = xg,
  fit = gam_pred$fit,
  lower = gam_pred$fit - 2 * gam_pred$se.fit,
